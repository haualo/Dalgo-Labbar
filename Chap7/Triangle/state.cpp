...
{
   return position>=0 && position<=14 && ((_state & (1<<position))!=0);
}
...
State State::nextState(const Move& move)
{
    ...
    State s = *this;
    s.clearPos( move._pos );
    s.clearPos( posNeighbour( move._pos, move._direction ) );
    s.setPos( posdestinationFromPosAndDirection( move._pos, move._direction ) );
    return s;
}// nextState
...
{
  int ix1   = move._pos;
  int ixMid = posNeighbour( ix1, move._direction );
  int ix2   = posNeighbour( ixMid, move._direction );
