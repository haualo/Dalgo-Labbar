...
void TriangleWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    for (Move move : moves){
        int pos2 = posdestinationFromPosAndDirection(_posSelected, move._direction );
...
void TriangleWidget::select(int pos)
{
    ...
    vector<Move> moves = _state.legalMoves(_posSelected);
    for (Move move : moves){
        int posNext =  posdestinationFromPosAndDirection( move._pos, move._direction);
        _posOfDestinations.insert(posNext);
        int posDisapear = posNeighbour(move._pos, move._direction);
...
void TriangleWidget::animate(Move move){
  ...
  _posSelected = move._pos;
  int posDestination = posdestinationFromPosAndDirection(move._pos, move._direction);
  int posDisapear    = posNeighbour(move._pos, move._direction);
